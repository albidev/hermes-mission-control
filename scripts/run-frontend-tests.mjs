 ...
 * are opt-in inventories: a test file missing from those lists — 18 of them were — runs in
 * no lane at all, and CI stays green while the file rots.
 *
 * Files are discovered by glob, never by list. Each runs as its own process (the repo
 * mixes `node:test` suites with plain assert-and-exit scripts).
 *
 ...
 * cannot hide inside the baseline.
 *
...
const runOne = (file) => new Promise((resolve) => {
  const child = spawn(
    process.execPath,
    ['--experimental-strip-types', join('tests', file)],
...
const passing = [];
const failing = [];
const baselineStillFailing = [];

for (const file of files) {
  const result = await runOne(file);
  if (result.code === 0) {
    passing.push(file);
    if (filter || process.argv.includes('--verbose')) console.log(`  ok   ${file}`);
    continue;
  }
  failing.push(result);
  if (known[file]) baselineStillFailing.push(file);
}
