...
export function ChatDrawer({ open, storedToken, initialSessionId, onClose }: ChatDrawerProps) {
  ...
  return (
    <>
      ...
      {open && isExpanded ? <WhiteboardPanel key={sessionId || 'new'} sessionId={sessionId} sessionTitle={headerSessionTitle} expanded={isExpanded} onClose={() => setIsExpanded(false)} /> : null}
    </>
  );
}
