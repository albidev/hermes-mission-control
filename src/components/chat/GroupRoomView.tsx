...
export function GroupRoomView({ state, onSend, className = '', mentionRoster = [] }: GroupRoomViewProps) {
  ...
  return <section className={`flex min-h-0 flex-col gap-2 ${className}`} aria-label={t('rooms.title')}>
    <header className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border-subtle bg-surface-raised/40 px-2.5 py-1.5">
      <div className="min-w-0"><p className="truncate text-sm font-semibold leading-tight text-text">{state.room?.name || t('rooms.title')}</p><p className="text-[10px] leading-tight text-text-muted">{state.room?.members.length ?? 0} {t('rooms.members').toLowerCase()}</p></div>
      <div className="flex items-center gap-1">
        {!state.disbanded && !state.serviceUnavailable ? <button type="button" onClick={() => void state.stopRoom()} disabled={state.refreshing} title={t('rooms.stop')} className="inline-flex min-h-7 items-center gap-1 rounded-md border border-border-subtle px-1.5 text-[11px] text-text-muted hover:bg-surface-sunken disabled:opacity-50"><XOctagon size={12} />{t('rooms.stop')}</button> : null}
        {!state.disbanded && !state.serviceUnavailable ? <button type="button" onClick={() => { if (confirmDisband) { setConfirmDisband(false); void state.disband(); } else { setConfirmDisband(true); window.setTimeout(() => setConfirmDisband(false), 3000); } }} title={t('rooms.disbandAction')} className={`inline-flex min-h-7 items-center gap-1 rounded-md border px-1.5 text-[11px] ${confirmDisband ? 'border-negative bg-negative text-white' : 'border-negative/30 text-negative hover:bg-negative-subtle'}`}>{confirmDisband ? <Check size={12} /> : <Trash2 size={12} />}{t(confirmDisband && state.room ? 'rooms.disbandConfirm' : 'rooms.disbandAction', confirmDisband && state.room ? { name: state.room.name || state.room.id } : {})}</button> : null}
