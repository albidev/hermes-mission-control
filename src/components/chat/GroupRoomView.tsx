...
export function GroupRoomView({ state, onSend, className = '' }: GroupRoomViewProps) {
  ...
  return <section className={`flex min-h-0 flex-col gap-3 ${className}`} aria-label="Group Chat room">
    ...
    {onSend ? <form onSubmit={submit} className="flex gap-2"><input value={draft} onChange={(event) => setDraft(event.target.value)} disabled={state.disbanded || state.serviceUnavailable} placeholder="Message the room…" className="min-w-0 flex-1 rounded-lg border border-border-subtle bg-surface px-3 py-2 text-sm text-text outline-none focus:border-accent" /><button type="submit" disabled={!draft.trim() || state.disbanded || state.serviceUnavailable} className="inline-flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-sm text-white disabled:opacity-50"><Send size={14} />Send</button></form> : null}
